...
Layout (one directory per campaign under ``campaigns/``)::
    ...
    campaigns/<CAMPAIGN-ID>/
        ...
        report.pdf       the rendered report (engine-specific)
...
class TestResult:
    """One test case (TC-01, CF-03, AMF-REG-01, NRF-DISC-01, ...)."""
    id: str
    name: str
    status: str                       # "pass" | "fail" | "measured" | "error" | "na" | "not_implemented"
...
@dataclasses.dataclass
class SuiteResult:
    suite: str                        # free-form suite label (performance | pfcp | amf | smf | ...)
...
class Store:
    def __init__(self, root: Path, campaign_id: str):
        ...
        self._sut_live: dict[str, Any] = {}  # facts read live from the adapter
    ...
    def set_sut_live(self, facts: dict[str, Any]) -> None:
        """Facts the adapter read live (mode, image, ports...). Merged over the
