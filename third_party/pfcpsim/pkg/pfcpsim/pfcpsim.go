	...
	for {
		...
		default:
			...
			case *message.HeartbeatRequest:
				// TS 29.244 §6.2.2: a node receiving a Heartbeat Request shall
				// answer with a Heartbeat Response carrying its Recovery Time
				...
				if err := c.sendMsg(hbRes); err != nil {
					logger.PfcpsimLog.Errorln("Error sending Heartbeat Response:", err)
				}
				continue
...
func (c *PFCPClient) SendAssociationTeardownRequest(ie ...*ieLib.IE) error {
	...
	teardownReq := message.NewAssociationReleaseRequest(
		c.getNextSequenceNumber(),
		ieLib.NewNodeID(c.localAddr, "", ""),
	)
...
func (c *PFCPClient) SendHeartbeatRequest() error {
	// Carry only the Recovery Time Stamp (pass nil for the Source IP Address IE).
