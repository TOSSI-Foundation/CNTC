...
// applyActionIE builds the Apply Action IE. go-pfcp emits the Rel-15 1-octet form,
// which OMEC/SD-Core and OAI accept; Open5GS's (Rel-16) parser requires 2 octets
// ("Invalid TLV length 1. It should be 2"). Set PFCPSIM_APPLY_ACTION_2B to emit the
// 2-octet form (action flags + a zero extension octet) for those stricter UPFs.
func applyActionIE(action uint8) *ie.IE {
	if os.Getenv("PFCPSIM_APPLY_ACTION_2B") != "" {
		return ie.NewApplyAction(action, 0x00)
	}
	return ie.NewApplyAction(action)
}
