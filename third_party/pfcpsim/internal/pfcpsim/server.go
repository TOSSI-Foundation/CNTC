...
func (P pfcpSimService) CreateSession(ctx context.Context, request *pb.CreateSessionRequest) (*pb.Response, error) {
	...
	if v := os.Getenv("PFCPSIM_MBR_KBPS"); v != "" {
		if n, perr := strconv.ParseUint(v, 10, 64); perr == nil {
			sessMBR, appUlMBR, appDlMBR = n, n, n
		}
	}
	...
	for i := baseID; i < (count*SessionStep + baseID); i = i + SessionStep {
		...
		// PFCPSIM_SINGLE_QER makes each PDR carry exactly one QER ID and the session
		// carry one QER, matching what Open5GS's 5G SMF sends. By default pfcpsim links a
		// session QER (id 0) plus a per-direction app QER to every PDR; Open5GS's strict
		// parser flags the extra QER ID ("Unknown TLV type [109]") and won't bind the QoS
		// flow, so its UPF never programs the gtp5g datapath. OMEC/SD-Core and OAI accept
		// the richer form, so the single-QER shape is only used when the env var is set.
		singleQER := os.Getenv("PFCPSIM_SINGLE_QER") != ""
		...
		if !singleQER {
			qers = append(qers,
				...
				session.NewQERBuilder().
					...
					WithUplinkMBR(sessMBR).
					WithDownlinkMBR(sessMBR).
					Build(),
			)
		}
		...
		for _, appFilter := range request.AppFilters {
			...
			ub := session.NewPDRBuilder().
				WithID(uplinkPdrID).
				WithMethod(session.Create).
				WithTEID(uplinkTEID).
				WithFARID(uplinkFarID)
			if !singleQER {
				ub = ub.AddQERID(sessQerID)
			}
			uplinkPDR := ub.
			...
			// Open5GS binds one QER per PDR; reuse the uplink app QER on the downlink PDR
			// (single-QER mode) instead of a separate downlink QER.
			dlQerID := downlinkAppQerID
			if singleQER {
				dlQerID = uplinkAppQerID
			}
			db := session.NewPDRBuilder().
			...
			uplinkAppQER := session.NewQERBuilder().
				...
				WithMethod(session.Create).
				WithQFI(qfi).
				WithUplinkMBR(appUlMBR).
				WithDownlinkMBR(appDlMBR).
		...
		// PFCPSIM_NO_URR=1 omits Usage Reporting Rules. URRs (VolumeQuota etc.) are
		// accepted by the OMEC/SD-Core BESS-UPF but rejected by stricter/leaner parsers
		// (e.g. OAI-UPF), which need only PDR/FAR/QER to forward. upfbench doesn't use
		// usage reporting, so this makes sessions portable across UPFs.
		if os.Getenv("PFCPSIM_NO_URR") != "" {
			urrs = nil
		}
